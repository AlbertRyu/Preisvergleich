...
async def create_product(
    name: str = Form(...), unit: str = Form("kg"), notes: str = Form("")
):
    with get_db() as db:
        cur = db.execute(
...
async def create_store(name: str = Form(...)):
    with get_db() as db:
        cur = db.execute(
            "INSERT OR IGNORE INTO stores (name) VALUES (?)", (name.strip(),)
        )
        store_id = (
            cur.lastrowid
            or db.execute(
...
async def compare(product_ids: str = ""):
    ...
    return {
        ...
        "products": [dict(p) for p in products],
        "grid": {
            str(k): {str(sk): sv for sk, sv in v.items()} for k, v in grid.items()
        },
